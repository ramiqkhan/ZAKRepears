...
import { 
  ...
  ChevronDown, CheckCircle2, Phone, Calendar, 
  ...
  Our <span className="bg-blue-600 px-4 py-1 inline-block rounded-xl">Services</span>
        ...
        <div className="max-w-4xl mx-auto relative z-10 space-y-4">
          {services.map((service, index) => (
            <div key={index} className={`bg-[#0a1221] rounded-2xl border transition-all ${activeAccordion === index ? 'border-blue-500/50 shadow-[0_0_30px_rgba(37,99,235,0.1)]' : 'border-white/5 hover:border-white/10'}`}>
              <button 
              ...
              >
                <div className="flex items-center gap-6">
                  ...
                  <h3 className="text-lg md:text-xl font-black uppercase tracking-wide">{service.title}</h3>
                </div>
              ...
              <div className={`transition-all duration-300 overflow-hidden ${activeAccordion === index ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="px-8 md:px-24 pb-10 grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-white/5 pt-6">
      ...
      {/* --- 2. BEFORE & AFTER GALLERY (Same Background Theme) --- */}
     

      {/* --- 3. TESTIMONIALS SECTION (Seamless Integration) --- */}
     

      {/* --- 4. CTA FOOTER --- */}
      <div className=" py-20 px-6 text-center">
         <div className="max-w-4xl mx-auto">
           <h2 className="text-4xl md:text-6xl font-black uppercase tracking-tighter mb-8">Ready To Start Your Project?</h2>
           <button className="bg-white text-blue-500 px-14 py-5 rounded-2xl font-black uppercase text-xl shadow-2xl hover:bg-gray-100 transition-all active:scale-95">
             Book Service Now
           </button>
         </div>
      </div>
